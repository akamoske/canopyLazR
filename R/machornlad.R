...
#' 
#' @param leveld.lidar.array.list List of leveled lidar arrays, which is the output of the canopy.height.levelr function
...
machorn.lad <- function(leveld.lidar.array.list, voxel.height, beer.lambert.constant = NULL){

  #create an empty list
  raw.lad.list <- list()
  
  #loop through the files for data management
  for (m in 1:length(leveld.lidar.array.list)) {
    
    #save the first file as a new variable
    lidar.array <- leveld.lidar.array.list[[m]]
    
    #first lets just use the pulse totals in each voxel, we do not need the ground or canopy elevations at this point
    voxel.N.pulse <- lidar.array$array[3:dim(lidar.array$array)[1],,]
    ...
    #array the ground is a value of 0 because no pulses traveld through this column. but the accumulation of pulses above it are the number of pulses
    ...
    #lets save this data into our list
    raw.lad.list[[m]] <- out
