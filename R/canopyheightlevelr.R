...
#' 
#' @param lidar.array.list List of voxelized arrays that was created with the las.to.array function
#' @return A list of leveled voxelized arrays
#' @export

canopy.height.levelr <- function(lidar.array.list){
  
  #create an empty list
  level.canopy.list <- list()
  
  #loop through the data for memory managment
  for(i in 1:length(lidar.array.list)) {
    
    #save the first file as a new variable
    lidar.array <- lidar.array.list[[i]]
    ...
    level.canopy.list[[i]] <- return.data
