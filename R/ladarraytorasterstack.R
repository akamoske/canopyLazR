...
lad.array.to.raster.stack <- function(lad.array.list, epsg.code) {
  ...
  for (z in 1:length(lad.array.list)) {
    ...
    for(f in 1:length(lad.mat)){
      ...
      lad.raster <- raster::raster(lad.mat[[f]],
                                   xmn = laz.data[[z]]$x.bin[1],
                                   xmx = laz.data[[z]]$x.bin[length(laz.data[[z]]$x.bin)],
                                   ymn = laz.data[[z]]$y.bin[1],
                                   ymx = laz.data[[z]]$y.bin[length(laz.data[[z]]$y.bin)],
                                   crs = crs.proj)
