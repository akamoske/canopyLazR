...
laz.to.array <- function(laz.files.path, voxel.resolution, z.resolution) {
  ...
  for (i in 1:length(laz.files.list)) {
    ...
    x.range.raw <- range(laz.xyz$x, na.rm = TRUE)
    y.range.raw <- range(laz.xyz$y, na.rm = TRUE)
    z.range.raw <- range(laz.xyz$z, na.rm = TRUE)
    ...
    x.bin <- seq(x.range[1], x.range[2], x.y.grain, na.rm = TRUE)
